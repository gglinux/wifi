		...
		try {
			...
			while (!storge.queue.isEmpty()) {
				...
				for (int i = 0; i < data.length; i++) {
					...
					System.out.println("[info:put] '" + this.name + rowKey
							+ "data " + data[i]);
				}
				// ///////////////////////////////////////////////////////
				...
				/*
				 * String[] data = storge.pop().getData(); String rowKey =
				 * data[0]+":"+System.nanoTime()+":"+data[1];
				 * 
				 * 
				 * Put p1=new Put(Bytes.toBytes(rowKey));
				 * 
				 * for(int i = 0; i < data.length; i++){
				 * p1.add(Bytes.toBytes(HBaseTable.familyName),
				 * Bytes.toBytes("col"+i), Bytes.toBytes(data[i]));
				 * StartInsert.list.add(p1);
				 * System.out.println("[info:add] '"+this
				 * .name+rowKey+"data "+data[i]); } if(StartInsert.list.size() >
				 * this.count) {
				 * 
				 * StartInsert.table.put(StartInsert.list);
				 * StartInsert.list.clear(); StartInsert.table.flushCommits();
				 * System
				 * .out.println("线程:"+Thread.currentThread().getId()+"插入数据："
